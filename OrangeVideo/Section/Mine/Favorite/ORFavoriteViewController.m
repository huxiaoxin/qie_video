...
@interface ORFavoriteViewController ()

@property (nonatomic, strong) ORFavoriteListDataModel *favoriteListDataModel;
@property (nonatomic, strong) ORDeleteFavoriteDataModel *deleteFavoriteDataModel;
...
@property (nonatomic, strong) NSMutableArray <__kindof ORFavoriteListDataItem *> *dataArr;
@property (nonatomic, strong) NSMutableArray <__kindof ORFavoriteListDataItem *> *deleteArr;
...
@implementation ORFavoriteViewController
...
+ (Class)cellClsForItem:(id)item
{
    if ([item isKindOfClass:[ORFavoriteListDataItem class]]) {
        return [ORFavoriteTableViewCell class];
    }
    return nil;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView.isEditing) {
        ...
        ORFavoriteListDataItem *cellItem = (ORFavoriteListDataItem *)[self.dataArr mdf_safeObjectAtIndex:indexPath.row];
        if (self.editView.hidden) {
            if ([cellItem.type isEqualToString:@"hotspot"]) {
                // 跳转到视频详情
                ORHotChannelDataItem *dataItem = [[ORHotChannelDataItem alloc] init];
                ...
                ORHotDetailViewController *hotDetailVC = [[ORHotDetailViewController alloc] initWithVideoModel:dataItem];
...
- (void)deleteBtnClick:(UIButton *)btn
{
    if (self.tableView.editing) {
        ...
        [self.deleteArr enumerateObjectsUsingBlock:^(__kindof ORFavoriteListDataItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSMutableDictionary *paramDic = [NSMutableDictionary dictionary];
            paramDic[@"id"] = obj.idStr;
            paramDic[@"type"] = obj.type;
            [tmpDeleteArr mdf_safeAddObject:paramDic];
        }];
        [self.deleteFavoriteDataModel deleteFavoriteWithData:tmpDeleteArr];
    }
}
...
- (void)handleDataModelSuccess:(GNHBaseDataModel *)gnhModel
{
    ...
    if ([gnhModel isMemberOfClass:[ORFavoriteListDataModel class]]) {
...
- (ORFavoriteListDataModel *)favoriteListDataModel
{
    if (!_favoriteListDataModel) {
        _favoriteListDataModel = [self produceModel:[ORFavoriteListDataModel class]];
    }
