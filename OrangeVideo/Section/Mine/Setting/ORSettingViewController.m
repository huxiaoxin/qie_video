...
@interface ORSettingViewController ()

@property (nonatomic, assign) BOOL isPushOn;

@end

@implementation ORSettingViewController
...
- (void)setupUI
{
    ...
    settingSectionObject.footerTableViewCellHeight = 19.5f;
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [settingSectionObject.items enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:[ORSettingCellItem class]]) {
                ORSettingCellItem *cellItem = (ORSettingCellItem *)obj;
...
- (UIView *)setupTableFooterView
{
    ...
    [logoutBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(footerView).inset(15.0f);
...
+ (Class)cellClsForItem:(id)item
{
    if ([item isKindOfClass:[ORSettingCellItem class]]) {
        return [ORSettingTableViewCell class];
...
- (void)configForCell:(GNHBaseTableViewCell *)cell item:(id)item
{
    ...
    if ([item isKindOfClass:[ORSettingCellItem class]]) {
        ...
        ORSettingTableViewCell *settingCell = (ORSettingTableViewCell *)cell;
        settingCell.settingActionBlock = ^(ORSettingCellType type, BOOL isOn) {
...
- (NSMutableArray *)dataItems
{
    ...
    ORSettingCellItem *cellItem1 = [[ORSettingCellItem alloc] init];
    cellItem1.cellType = ORSettingCellTypeClearMemory;
    cellItem1.title = @"清除缓存";
        
    ORSettingCellItem *cellItem2 = [[ORSettingCellItem alloc] init];
    cellItem2.cellType = ORSettingCellTypePush;
    cellItem2.title = @"消息推送";
    cellItem2.isOn = [self fetUserNotification];
        
    ORSettingCellItem *cellItem3 = [[ORSettingCellItem alloc] init];
    ...
    ORSettingCellItem *cellItem4 = [[ORSettingCellItem alloc] init];
    ...
    ORSettingCellItem *cellItem5 = [[ORSettingCellItem alloc] init];
...
- (void)logoutAction:(UIButton *)button
{
    [self gnh_showAlertWithTitle:@"温馨提示" message:@"您确定需要退出登录吗？" appearanceProcess:^(GNHAlertController *alertMaker) {
        ...
        if (index == 1) {
            ...
            [ORAccountComponent checkLogin:YES];
        }
    }];
}
