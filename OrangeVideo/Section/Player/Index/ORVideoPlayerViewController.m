...
#import "ORVideoRecommendTableViewCell.h"
#import "ORVideoDetailTableViewCell.h"
#import <SJVideoPlayer/SJVideoPlayer.h>
#import <SJUIKit/NSAttributedString+SJMake.h>
#import <SJBaseVideoPlayer/SJPlaybackRecordSaveHandler.h>
#import <AVKit/AVPictureInPictureController.h>
#import "ORVideoPlayerDataModel.h"
#import "ORVideoIntroView.h"
#import "ORVideoEpisodesView.h"
...
@interface ORVideoPlayerViewController ()

@property (nonatomic, strong) ORVideoPlayerDataModel *videoPlayerDataModel; // 视频详情
@property (nonatomic, strong) ORVideoRecommendDataModel *videoRecommendDataModel; // 推荐视频数据
...
@property (nonatomic, strong) ORVideoModel *videoModel;
...
@property (nonatomic, strong) ORChooseSourceView *chooseSourceView;
...
@implementation ORVideoPlayerViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    GNHWeakSelf;
    // 查询最后更新的一集
    [self.videoItems enumerateObjectsUsingBlock:^(__kindof ORVideoModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
{
    if ([item isKindOfClass:[ORVideoDetailItem class]]) {
        return [ORVideoDetailTableViewCell class];
    } else if ([item isKindOfClass:[ORVideoRecommendItem class]]) {
        return [ORVideoRecommendTableViewCell class];
    }
...
{
    if ([item isKindOfClass:[ORVideoRecommendItem class]]) {
        GNHWeakSelf;
        ORVideoRecommendTableViewCell *videoRecommedCell = (ORVideoRecommendTableViewCell *)cell;
        videoRecommedCell.recommendItemCallBack = ^(ORVideoBaseItem * _Nonnull dataItem) {
            // 记录观看记录，存储前一次观看记录
...
- (void)setupData
{
    ...
    // 观看历史记录
    self.videoItems = [[ORVideoReportManager sharedInstance] videoItems];
    ...
    // 建立记录模型
    ORVideoModel *videoModel = [[ORVideoModel alloc] init];
...
- (void)storeWatchRecord
{
    ...
    if (ORShareAccountComponent.accesstoken.length) {
        ...
        [[ORVideoReportManager sharedInstance] videoReportWithData:self.videotype videoId:self.videoId watchSeconds:self.player.currentTime];
    }
    ...
    [[ORVideoReportManager sharedInstance] restoreVideoHistory:self.videoModel];
}
...
- (void)throwScreen:(UIButton *)btn
{
    ...
    // 投屏
    ORThrowScreenViewController *throwScreenController = [[ORThrowScreenViewController alloc] init];
...
- (void)playerAction:(UIButton *)btn
{
    if (self.videoPlayerDataModel.videoDetailItem.source.redirectType) {
        ...
        UIViewController *vc = [[ORH5ViewController alloc] initWithURL:self.videoPlayerDataModel.videoDetailItem.source.redirectLink.urlWithString];
...
{
    LYCoverView *videoIntroView = [ORVideoIntroView showIntroAlertView:self.videoPlayerDataModel.videoDetailItem completeBlock:^{
        // code
    }];
...
{
    GNHWeakSelf;
    LYCoverView *videoEpisodesView = [ORVideoEpisodesView showIntroAlertView:self.videoPlayerDataModel.videoDetailItem completeBlock:^(NSInteger index) {
        // 播放第几集
...
- (void)chooseSource
{
    if (self.chooseSourceView) {
        ...
        GNHWeakSelf;
        ORChooseSourceView *chooseSourceView = [[ORChooseSourceView alloc] initWithFrame:CGRectZero sources:self.videoPlayerDataModel.videoDetailItem.sourceList selectSource:self.currentSourceName];
...
- (void)handleDataModelSuccess:(GNHBaseDataModel *)gnhModel
{
    ...
    if ([gnhModel isMemberOfClass:[ORVideoPlayerDataModel class]]) {
        [self configData];
    ...
    } else if ([gnhModel isMemberOfClass:[ORVideoRecommendDataModel class]]) {
        [self stopPullDownToRefreshAnimation];

        GNHTableViewSectionObject *sectionObject = self.sections.firstObject;
        
        ORVideoRecommendItem *recommendItem = [[ORVideoRecommendItem alloc] init];
...
- (void)handleDataModelError:(GNHBaseDataModel *)gnhModel
{
    ...
    if ([gnhModel isMemberOfClass:[ORVideoRecommendDataModel class]]) {
        [self stopPullDownToRefreshAnimation];
    }
}
...
- (ORVideoPlayerDataModel *)videoPlayerDataModel
{
    if (!_videoPlayerDataModel) {
        _videoPlayerDataModel = [self produceModel:[ORVideoPlayerDataModel class]];
    }
    return _videoPlayerDataModel;
}

- (ORVideoRecommendDataModel *)videoRecommendDataModel
{
    if (!_videoRecommendDataModel) {
        _videoRecommendDataModel = [self produceModel:[ORVideoRecommendDataModel class]];
    }
