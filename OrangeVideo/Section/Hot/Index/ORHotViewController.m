...
@interface ORHotViewController () <VTMagicViewDataSource, VTMagicViewDelegate, VTMagicReuseProtocol>
...
@implementation ORHotViewController
...
- (void)setupUI
{
    ...
    [self.magicController.view mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.view).offset(kGNHStatusBarHeight);
    ...
    if (self.viewControllerAppearAtFirstTime) {
        ...
        [self hasNetwork:^(bool has) {
            if (!has) {
                ...
                // 获取频道分类
                [weakSelf.fetchChannelMenuDataModel fetchChannelMenuWithScene:@"hotspot"];
...
- (UIViewController *)magicView:(VTMagicView *)magicView viewControllerAtPage:(NSUInteger)pageIndex
{
    ...
    ORHotChannelViewController *detailViewController = [magicView dequeueReusablePageWithIdentifier:pageId];
    if (!detailViewController) {
        detailViewController = [[ORHotChannelViewController alloc] init];
    }
