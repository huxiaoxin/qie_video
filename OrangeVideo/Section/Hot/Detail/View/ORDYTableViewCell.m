...
@property (nonatomic, strong) ORLikeView            *likeView;    // 点赞
@property (nonatomic, strong) ORHotVideoItemButton   *shareBtn;    // 分享
@property (nonatomic, strong) ORSliderView          *sliderView;
...
@implementation ORDYTableViewCell
...
- (void)setVideoItem:(ORHotChannelDataItem *)videoItem
{
    _videoItem = videoItem;
    
    [self.playerSuperImageView sd_setImageWithURL:videoItem.coverImg.urlWithString];
    self.nameLabel.text = @"@熊猫追剧";
    self.contentLabel.text = videoItem.videoDesc;
    
    [self.likeView setupLikeState:videoItem.like];
    [self.likeView setupLikeCount:@(videoItem.likes).stringValue];
...
- (void)setDelegate:(id<ORHotVideoControlViewDelegate>)delegate
{
    _delegate = delegate;
}
...
{
    if (!_likeView) {
        _likeView = [ORLikeView new];
        GNHWeakSelf;
        _likeView.likeViewAction = ^(BOOL isLike) {
...
- (ORSliderView *)sliderView
{
    if (!_sliderView) {
        _sliderView = [ORSliderView new];
