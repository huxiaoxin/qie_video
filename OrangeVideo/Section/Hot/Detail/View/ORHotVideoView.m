...
#import "ORDYTableViewCell.h"
#import <MJRefresh.h>

#define ORDataSize  20

@interface ORHotVideoView()<ORHotVideoControlViewDelegate, UITableViewDataSource, UITableViewDelegate, SJPlayerAutoplayDelegate>

@property (nonatomic, strong) SJVideoPlayer *player;

@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) NSMutableArray <__kindof ORHotChannelDataItem *> *videos;
...
@property (nonatomic, strong) ORDoubleLikeView          *doubleLikeView;
...
//    self.player.defaultEdgeControlLayer.bottomMargin = 60.0f;
    ...
    self.player.playbackObserver.assetStatusDidChangeExeBlock = ^(__kindof SJBaseVideoPlayer * _Nonnull player) {
        ...
        if ( !self ) return;
        ORDYTableViewCell *currentCell = [self.tableView cellForRowAtIndexPath:self.tableView.sj_currentPlayingIndexPath];
    ...
    self.player.playbackObserver.currentTimeDidChangeExeBlock = ^(__kindof SJBaseVideoPlayer * _Nonnull player) {
        ORDYTableViewCell *currentCell = [weakSelf.tableView cellForRowAtIndexPath:weakSelf.tableView.sj_currentPlayingIndexPath];
...
- (void)setupTableView
{
    ...
    // 注册cell
    [ORDYTableViewCell registerWithTableView:tableView];
...
{
    ORDYTableViewCell *cell = [ORDYTableViewCell cellWithTableView:tableView indexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(ORDYTableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
}
...
- (void)sj_playerNeedPlayNewAssetAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // 隐藏播放按钮 和 播放器底部控制层
    ORDYTableViewCell *currentCell = [self.tableView cellForRowAtIndexPath:indexPath];
...
// 获取当前播放内容的索引
- (NSInteger)indexOfModel:(ORHotChannelDataItem *)model
{
    __block NSInteger index = 0;
    [self.videos enumerateObjectsUsingBlock:^(ORHotChannelDataItem *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (isSuccess) {
            ...
            if ([videoList isKindOfClass:[NSArray class]]) {
                for (NSDictionary *dict in videoList) {
                    ORHotChannelDataItem *model = [ORHotChannelDataItem yy_modelWithDictionary:dict];
...
- (NSMutableArray<__kindof ORHotChannelDataItem *> *)videos
{
    if (!_videos) {
...
- (ORDoubleLikeView *)doubleLikeView
{
    if (!_doubleLikeView) {
        _doubleLikeView = [ORDoubleLikeView new];
    }
