//
//  ORAccountNotificationManager.m
//  ZhangYuSports
//
...
#import "ORAccountNotificationManager.h"

@interface ORAccountInnerObserver : NSObject
@property (nonatomic, weak) NSObject<ORAccountObserver> *observer;
@end

@implementation ORAccountInnerObserver
@end

@interface ORAccountNotificationManager ()
...
@implementation ORAccountNotificationManager
...
{
    if (observer && [observer conformsToProtocol:@protocol(ORAccountObserver)]) {
        [_lock lock];
...
- (BOOL)isObserverExist:(NSObject<ORAccountObserver> *)observer
{
    NSMutableArray *observers = [self.observers mutableCopy];
    for (ORAccountInnerObserver *innerObserver in observers) {
